...
// ContainerWrapper 容器通用包装器，设置数据属性用于前端样式
...
// ImageHtml 生成图片 HTML
func ImageHtml(url, alt string, class ...string) HTMLComponent {
	return Img("").Attr("src", url).Alt(alt).Class(class...)
}

// StringsToOptions 将字符串切片转为 SelectBuilder 所需的 DefaultOptionItem 切片
func StringsToOptions(items []string) []shadcn.DefaultOptionItem {
	opts := make([]shadcn.DefaultOptionItem, len(items))
	for i, item := range items {
		opts[i] = shadcn.DefaultOptionItem{Text: item, Value: item}
	}
	return opts
}
